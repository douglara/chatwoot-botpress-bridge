...
class SendToBotpressTest < ActionDispatch::IntegrationTest
  ...
  test "valid event" do
    ...
    assert_equal true, Botpress::SendToBotpress.call(event: @event, botpress_endpoint: @botpress_endpoint, botpress_bot_id: @botpress_bot_id).success?
  end

  test "invalid event" do
    assert_raise(Exception) { Botpress::SendToBotpress.call(event: {}, botpress_endpoint: @botpress_endpoint, botpress_bot_id: @botpress_bot_id).success? }
  end

  test "invalid endpoint" do
    stub_request(:post, Regexp.new(@botpress_endpoint)).
    to_return(status: 404)
    result = Botpress::SendToBotpress.call(event: @event, botpress_endpoint: @botpress_endpoint, botpress_bot_id: @botpress_bot_id)
